      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-2">
          ...
          <>
            ...
            {editingProduct && (
              <div className="bg-white rounded-2xl p-4 shadow space-y-3">
                <h3 className="font-bold text-gray-800">{editingProduct.id ? 'Produkt bearbeiten' : 'Neues Produkt'}</h3>
                <div className="flex gap-2">
                  <input
                    value={editingProduct.icon}
                    onChange={(e) => setEditingProduct({ ...editingProduct, icon: e.target.value })}
                    className="w-14 text-center text-2xl border border-gray-300 rounded-xl p-2"
                    placeholder="🛍️"
                  />
                  <input
                ...
                <div className="flex gap-2">
                  <button onClick={() => setEditingProduct(null)} className="flex-1 py-2 rounded-xl bg-gray-100 text-gray-700 font-medium">Abbrechen</button>
            ...
            {/* Product list */}
            {(products ?? []).map((p) => (
              <div key={p.id} className={`bg-white rounded-2xl px-3 py-2 shadow flex items-center gap-2 ${!p.isActive ? 'opacity-50' : ''}`}>
                ...
                <div className="flex gap-1 items-center">
                  ...
                  <button onClick={() => toggleSoldOut(p)} className={`text-xs px-2 py-1 rounded-lg font-medium ${p.isSoldOut ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'}`}>
                    {p.isSoldOut ? 'Aus' : 'OK'}
                  </button>
                  <button onClick={() => toggleActive(p)} className={`text-xs px-2 py-1 rounded-lg font-medium ${p.isActive ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-500'}`}>
                    {p.isActive ? 'Aktiv' : 'Inaktiv'}
                  </button>
                  <button onClick={() => setEditingProduct(p)} className="text-blue-600 px-2 py-1 text-sm">✏️</button>
                </div>
              </div>
            ))}
          </>
        ...
        {tab === 'categories' && (
          <>
            ...
            {editingCategory && (
              <div className="bg-white rounded-2xl p-4 shadow space-y-3">
                <h3 className="font-bold text-gray-800">{editingCategory.id ? 'Kategorie bearbeiten' : 'Neue Kategorie'}</h3>
                <input
                ...
                />
                <div>
                  <label className="text-xs text-gray-500 block mb-1">Farbe</label>
                  <div className="flex gap-2 flex-wrap">
                    {CATEGORY_COLORS.map((col) => (
                      <button
                        key={col}
                        onClick={() => setEditingCategory({ ...editingCategory, color: col })}
                        className={`w-9 h-9 rounded-full border-4 transition-transform ${editingCategory.color === col ? 'border-gray-800 scale-110' : 'border-transparent'}`}
                ...
                <div className="flex gap-2">
                  <button onClick={() => setEditingCategory(null)} className="flex-1 py-2 rounded-xl bg-gray-100 text-gray-700 font-medium">Abbrechen</button>
            ...
            {(categories ?? []).map((cat) => (
              <div key={cat.id} className="bg-white rounded-2xl px-4 py-3 shadow flex items-center gap-3">
                ...
                <button onClick={() => setEditingCategory(cat)} className="text-blue-600 text-sm px-2">✏️</button>
                <button onClick={() => deleteCategory(cat.id!)} className="text-red-500 text-sm px-2">🗑️</button>
              </div>
            ))}
          </>
        )}
        ...
        {tab === 'user' && (
          <div className="bg-white rounded-2xl p-4 shadow space-y-3">
            <h3 className="font-bold text-gray-800">Benutzername</h3>
            <input
              value={userName}
